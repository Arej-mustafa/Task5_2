...
fetch('https://restcountries.com/v3.1/all')
  ...
  .then(function (data) {
    ...
    data.forEach((item) => {
      ...
      listItem.innerHTML = `<img style="width: 20px;" src="${item.flags.svg}" alt="${item.vca2}" /> ${item.cca2}`;
      listItem.addEventListener("click", () => {
        selectedCountry.innerHTML = `<img style="width: 10px;" src="${item.flags.svg}" alt="${item.cca2}" /> ${item.cca2}`;
        countryMenu.style.display = "none";
      });
      countryMenu.appendChild(listItem);
    });
